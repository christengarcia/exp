...
# -*- coding: utf-8 -*-
"""
This program allows the user to choose an unlimited number 
...
def tax_deductible(question):
    while "the answer is invalid":
        ...
        if reply == "Yes" or reply == "yes":
            ...
            print("The options are Yes or No")            

# prompt user for the number of items to be recorded
items = get_non_negative_int("How many items do you wish to record? ")
items = int(items)    
...
while number_of_items < items:
    ...
    tax = tax_deductible("Is this item tax deductible (Yes,No)? ")
    
    # multiply item cost and items used. Then add result to items or tax list respectively
    total = (item_cost * items_used)
    items_list.append(total)
    tax_list.append(tax) 
    
    # add most expensive item with item number to dictionary
